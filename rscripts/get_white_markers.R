...
samples[which(data@meta.data$type == 'brown')] <- 'brown' #replace brown sample names by 'brown'
...
write.table(df.cluster_markers, file=paste(opt$outdir, 'markers_', output_prefix, '_white', sep=''), sep='\t', row.names=F, quote=F)
