...
print('running pca')
seurobj <- RunPCA(seurobj, pcs.compute=30, do.print=F)


print('running clustering')
for (res in resolutions){
  seurobj <- FindClusters(seurobj, reduction.type = "pca", dims.use = 1:20, resolution = res, print.output = 0, save.SNN = TRUE)
}
