...
```{r}
FeaturePlot(seurobj, reduction.use='pca', features.plot=c('EBF2'), pt.size=1, cols.use=c('gray', 'blue'), no.legend=F)
```
...
FeaturePlot(seurobj, reduction.use='tsne18',  features.plot=as.vector(markers.time.1[order(-markers.time.1$avg_logFC),'gene'][1:10]), cols.use=c('gray', 'blue'), no.legend=F, nCol=2)
```
...
```{r fig13, fig.height=30, fig.width=15, fig.align="center"}
FeaturePlot(seurobj, reduction.use='tsne18', features.plot=c('PLA2G2A', 'SAA1', 'MT1X', 'APOD', 'DPT', 'CST3', 'PTGDS', 'IGF2', 'THBS1', 'TM4SF1'), cols.use=c('gray', 'blue'), no.legend=F, nCol = 2)
```
